...
namespace System.Collections.Generic
{
    ...
    {
        private const int __Array_MaxLength = 1 << 30;
        ...
        {
            _nodes = EnumerableHelpers.ToArray(items ?? throw new ArgumentNullException(nameof(items)), out _size);
            _comparer = InitializeComparer(comparer);
        ...
        {
            return _size == 0 ? throw new InvalidOperationException() : _nodes[0].Element;
        }
        ...
        public void EnqueueRange(IEnumerable<(TElement Element, TPriority Priority)> items)
        {
            ...
            if (!(collection is null) && (count = collection.Count) > _nodes.Length - _size)
            {
                Grow(_size + count);
            }
        ...
        private void Grow(int minCapacity)
        {
            ...
            if ((uint)newcapacity > __Array_MaxLength)
            {
                newcapacity = __Array_MaxLength;
            }
